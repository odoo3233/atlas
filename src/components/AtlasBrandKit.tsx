...
export const AtlasButton: React.FC<AtlasButtonProps> = ({
  ...
  const baseClasses =
    "font-semibold transition-all duration-300 rounded-lg inline-flex items-center justify-center";

  const variants = {
    primary:
      "bg-gradient-to-r from-atlas-brown-800 to-atlas-gold-600 text-white hover:from-atlas-brown-700 hover:to-atlas-gold-500 shadow-lg hover:shadow-xl",
    secondary:
      "bg-atlas-gold-500 text-atlas-brown-900 hover:bg-atlas-gold-400 shadow-md hover:shadow-lg",
    outline:
      "border-2 border-atlas-gold-600 text-atlas-gold-600 hover:bg-atlas-gold-600 hover:text-white",
    ghost:
      "text-atlas-brown-700 hover:bg-atlas-gold-100 hover:text-atlas-brown-900",
  };
...
export const AtlasSpinner: React.FC<{
  size?: "sm" | "md" | "lg";
  className?: string;
}> = ({ size = "md", className = "" }) => {
  const sizes = {
...
// Brand Divider
export const AtlasDivider: React.FC<{
  className?: string;
  gradient?: boolean;
}> = ({ className = "", gradient = false }) => {
  return (
    <div
      className={`
        h-px w-full my-8
...
// Brand Icon Button
interface AtlasIconButtonProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement> {
...
export const AtlasIconButton: React.FC<AtlasIconButtonProps> = ({
  ...
  const variants = {
    default:
      "bg-atlas-brown-800 text-white hover:bg-atlas-brown-700 shadow-md hover:shadow-lg",
